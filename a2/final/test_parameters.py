...
if __name__ == "__main__":
	...
	parser.add_argument("--clf_cmd",help="What is the command in python of the clf you want to use? Note: if you specify a clf file then don't specify it's name. Options are: \n" + "\n".join([str(c) for c in classifiers]),nargs="*",default=None)
	...
	if not args.test_data and args.feature_reduction:
		print parser.print_help()
		raise RuntimeError("You can't specify a type of feature reduction without test data. You must load a selector with --feature_selection_file or provide train data (--train_data) to train the selector")
	...
	if args.train_data:
		...
		X_train = load_X(args.train_data)
